...
// export lemmatta 3.5.2 and 3.5.5 data into .csv file
if (lemmattaInfo ne []) then
    strLem:= "";
    for i in [1..#lemmattaInfo] do
	strLem:= strLem cat lemmattaInfo[i];
	if (i ne #lemmattaInfo) then
	    strLem:= strLem cat ",";
	end if;
    end for;
    fprintf Lemmatta, hash cat "," cat strLem cat "\n";
end if;
...
powerbasis:= [];
for i in [1..n] do
    powerbasis[i]:= Eltseq(IntegralBasisK[i]);
end for;
exitline:= "";
for C in [1..#alphgamlist] do
    strPrimelist:= SeqEnumToString(alphgamlist[C]`primelist);
    aduset:= alphgamlist[C]`adu;
    strAdu:= "\"(";
    for i in [1..#aduset] do
	adu:= aduset[i];
	strAdu:= strAdu cat SeqEnumToString(adu: Quotations:= false);
	if (i ne #aduset) then
	    strAdu:= strAdu cat ",";
	end if;
    end for;
    ...
    nalpha:= [];
    for i in [1..n] do
	nalpha[i]:= &+[alpha[j]*powerbasis[j][i] : j in [1..n]];
    end for;
    assert K!nalpha eq K!alpha;
    strAlpha:= SeqEnumToString(nalpha);
    if (C ne #alphgamlist) then
	exitline:= exitline cat hash cat "," cat strAdu cat strPrimelist cat "," cat
		   strAlpha cat "\n";
    else
	exitline:= exitline cat hash cat "," cat strAdu cat strPrimelist cat "," cat
		   strAlpha;
