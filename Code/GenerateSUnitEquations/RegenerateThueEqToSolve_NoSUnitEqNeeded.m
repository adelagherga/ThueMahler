...
Description: This program iterates through "NoSUnitNeeded.csv" to
	     regenerate all Thue equations. The output is printed in
             "RegeneratedThueEqToSolve.csv" and will supercede the file "ThueEqToSolve.csv"
	     generated by "GenerateSUnitEquations.m", which is now rendered obsolete.

             This update to "ThueEqToSolve.csv" is necessary to override output errors in the
	     previous same-named file, as generated by "GenerateSUnitEquations.m", where
...
prep0:= function(N,clist)
   ...
   */
	...
	enterTM:= false;
	return f,enterTM,RemainingCasesAllAs,ThueToSolve;
    end if;
    ...
    assert enterTM;
    return f,enterTM,remainingCase,ThueToSolve;
...
SUnitErr:= "/home/adela/ThueMahler/Data/SUnitEqData/RegeneratedSUnitErr.txt";

// .csv format is
// N,"form","optimal form","RHSlist"
// optimal form is also Thue equation to solve
ThueEqToSolve:= "/home/adela/ThueMahler/Data/SUnitEqData/RegeneratedThueEqToSolve.csv";
...
// delimiter for optimal form; may be "None"
assert (#RBracketSplit eq 3) or (#RBracketSplit eq 5);
...
if (#RBracketSplit eq 3) then
    assert CommaSplit[6] eq "None";
    // convert bash input for form into a sequence of integers
    clist:= [StringToInteger(i) : i in Split(RBracketSplit[2],",")];
    NoThueEq:= StringToInteger(Split(RBracketSplit[3],",")[3]);
    assert NoThueEq eq StringToInteger(CommaSplit[7]);
else
    ...
    NoThueEq:= StringToInteger(Split(RBracketSplit[5],",")[2]);
    assert NoThueEq eq StringToInteger(CommaSplit[10]);
end if;

// add original form to hash in .csv format
hash:= hash cat "\"(" cat RBracketSplit[2] cat ")\",";
assert hash eq &cat[set[i] : i in [1..#hash]];
// add (optimal) clist to hash in .csv format
hash:= hash cat SeqEnumToString(clist);

// print out hash in LogFile in the event of errors
printf hash cat "\n";

f,enterTM,remainingCase,ThueToSolve:= prep0(N,clist);
...
if (enterTM ne false) then
    ...
    Integerf:=&+[fclist[i+1]*x^(n-i) : i in [0..n]];
    assert f eq Integerf;
    ...
    if (IsEmpty(afplist) ne false) then
	alphgamlist:= prep2(fieldKinfo,ClK,afplist);
	assert IsEmpty(alphgamlist);
    end if;
end if;
