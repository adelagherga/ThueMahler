// VERSION UPDATED ON 05/17: (ADELA)

// Updated to remove error of computing image of theta_i in C as 0 (04/17)
// Updated to compute missing solutions (see eg. C:= [ 6, 3, 12, -7 ], p:=[2,3], A:= 1)
...
while DoneAllCases eq false do
    ...
    PrintFile(OutFile, "Found a Solution!");
    PrintFile(OutFile, SOL1);
...
// ADELA FUCKING AROUND, changing to create correct setting of C_0 to 1 at start of code;
intrinsic SetC0To1(C::[RngIntElt],p::[RngIntElt],a::RngIntElt) -> SeqEnum
    {}
    n:=#C-1;
    v:=#p;
    ...
    for i in [1..v] do
        Append(~a_i, [0..Valuation(C[0+1],p[i])]);
    end for;
    
    for i in [1..#q] do
        Append(~b_i, [0..Min( Valuation(a,q[i]) , Valuation(C[0+1],q[i]) )] );
    end for;
    ...
    for i in [1..#D] do
        ...
        new_a:=( C[0+1]/d )^(n-1)*a*(1/d);
        
        for j in [1..v] do
            b_d[i][2][j]:= Valuation(new_a,p[j]);   // stores ord_{p_i}(a) to ensure gcd(a,p_1, ..., p_v) = 1
            new_a:= new_a/( p[j]^b_d[i][2][j] );        // divides out factors of p_i from a; ensures gcd(a,p_1, ..., p_v) = 1
        end for;
        a_d[i]:= new_a;
...
// ADELA: initial setup
    ...
    a:= A;
    n:= #C-1;       // computes degree of F(x,y) = C[0+1]x^n + C[2]x^{n-1}y + \cdots + C[n]xy^{n-1} + C[n+1]y^n; NB. C = [ C[1], ..., C[n+1] ] where C[i+1] denotes c_i
    v:= #p;         // computes the number of primes F(x,y) = a * p_1^{z_1} \cdots p_v^{z_v}
    originalp:= p;
    ...
    for i in [1..n] do
        c[i]:=C[i+1]*C[0+1]^(i-1);  // c_i = C[i+1]*C[1]^{i-1} for i = 1, ..., n; used to reduce to the case where c_0 = 1
    end for;
    
    for i in [1..originalv] do
        b[i]:= Valuation(a,p[i]);   // stores ord_{p_i}(a) to ensure gcd(a,p_1, ..., p_v) = 1
        a:= a/( p[i]^b[i] );        // divides out factors of p_i from a; ensures gcd(a,p_1, ..., p_v) = 1
    end for;
    
    a:= Integers()! a;

    if FIsIrreducible(c) eq false then
        print("F is reducible. The algorithm requires F to be irreducible.");   // NB. if F(X,Y) is reducible, the algorithm terminates completely
        return [[-1]];
    end if;
    ...
    F:= C[1]*X^3 +C[2]*X^2*Y + C[3]*X*Y^2 + C[4]*Y^3;   // generates polynomial F
    ...
    for D in nD do
        ...
        if LacksLocalSolutions then 
            return [[-1]]; 
        end if;

        //print "OriginalIndicesOfRemovedPrimes:", OriginalIndicesOfRemovedPrimes;
        ...
        for b2 in b_d do
            ...
            for sol1 in SOL1 do
                ...
                if IsDivisibleBy(d*sol1[1],C[0+1]) then
                    //x:=sol1[1]/C[0+1];    
                    sol2[1]:=Integers() ! (d*sol1[1]/C[0+1]);
                   ...
                   // print sol2[1];
                    ...
                    /*for k:=1 to originalv do
                        ...
                        if k in OriginalIndicesOfRemovedPrimes then
                            ...
                            sol2[2+k]:=0+bd[k]-b[k];
                            if sol2[2+k] lt 0 then 
                                continue sol1; 
                            end if;

                        else

                            sol2[2+k]:=sol1[2+i]+bd[k]-b[k];
                            i+:=1;
                            if sol2[2+k] lt 0 then 
                                continue sol1; 
                            end if;
                    ...
                    */
                    //print "sol2", sol2;
                    //if GCD(sol2[1], sol2[2]) eq 1 then
                    
                    // HACK TO FIX EXPONENTS:
                    if Evaluate(F,[sol2[1],sol2[2]]) ne 0 then
                        Fac:= Factorization(Evaluate(F,[sol2[1],sol2[2]]));       
                        Fac1:= [f[1] : f in Fac];   // primes appearing in Factorization of RHS of F
                        Fac2:= [f[2] : f in Fac];   // exponents on primes appearing on RHS of F
...
/*
INPUT:
    c:= [c_1,...,c_n], the integer sequence of coefficients of F(x,y), where
        F(X,Y) = X^n + c[1]X^{n-1}Y + \cdots + c[n]Y^n 
        NB. The coefficient c_0 of X^n is not to be entered; it is assumed that c_0 = 1
        
OUTPUT:
    if F(X,Y) is reducible: 
        Returns true
    if F(X,Y) is irreducible: 
        Returns false
    
COMMENTS:
    If F(X,Y) is not reducible, the code exits here
...
{}
    ...
    n:= #c;     // determines the degree of F(X,Y) 
    Zxy<x,y>:= PolynomialRing(IntegerRing(), 2); 
    F:= x^n + c[n]*y^n;
    for i in [1..n-1] do
        F:= F + c[i]*x^(n-i)*y^i;       // generates F(X,Y)
    end for;
...
INPUT:
    c:= [c_1,...,c_n], the integer sequence of coefficients of F(x,y), where
        F(X,Y) = X^n + c[1]X^{n-1}Y + \cdots + c[n]Y^n 
            It is assumed that F(X,Y) is irreducible
        NB. The coefficient c_0 of X^n is not to be entered; it is assumed that c_0 = 1
    p:= [p[1], ..., p[v]], the sequence of rational primes, p[i]= p_i, where
        F(X,Y) = a * p_1^{z_1} \cdots * p_v^{z_v}
    a:= integer such that F(X,Y) = a * p_1^{z_1} \cdots * p_v^{z_v}
        NB. it is assumed that (a,p_i)=1 for i= 1,..,v
    LogFileOutFile:= [ LogFile, OutFile ], where
        LogFile:= a string giving the name of the log file; this file logs the key steps of the algorithm and its results at those key steps
        OutFile:= a string giving the name of the output file; this file stores the solutions to the Thue-Mahler equation
    FindAll:= a boolean variable. 
        if FindAll == false: the function only finds the small solutions of the Thue-Mahler equation
        if FindAll == true: the function finds all solutions of the Thue-Mahler equation
    StartEndCase:= TO BE FILLED!
    
OUTPUT:
    TO BE FILLED!! The list of solutions [X,Y,z_1,...,z_v] of the Thue-Mahler equation with gcd(X,Y)=1.
    
COMMENTS:
    This algorithm is based primarily on Tzanakis, de Weger's "How to explicitly solve a Thue-Mahler equation"
...
StartCase:=StartEndCase[1];
EndCase:=StartEndCase[2];
    LogFile:= LogFileOutFile[1];    // sets LogFile
    OutFile:= LogFileOutFile[2];    // sets OutFile

    n:= #c;     // defines n, the degree of F(t,1) =g(t)
    v:= #p;     // defines the number of primes  p_1, ..., p_v such that F(X,Y) = a * p_1^{z_1} \cdots * p_v^{z_v} 
...
//ExceptionalTuplesTest:=[];
    ...
    g:=t^n+c[n];
    for i in [1..n-1] do
        g:= g+c[i]*t^(n-i);     // defines g(t) = F(t,1) = t^n + c[1]*( t^{n-1} ) + \cdots + c[n-1]*( t ) + c[n] 
    end for;

    DiscriminantOfg:= Discriminant(g);  // computes the discriminant of g

    K<theta>:= NumberField(g);  // defines K = Q(theta) where theta is a root of g(t)
    OK:= RingOfIntegers(K);     // generates the ring of integers O_K of K 
    IntegralBasisK:= IntegralBasis(K);  // compute the integral basis for K

    CoefficientsOfIntegralBasisElement:= [];
    for i in [1..n] do
        CoefficientsOfIntegralBasisElement[i]:= Eltseq(IntegralBasisK[i]);      // writes each element of the integral basis of K in terms of the power basis of K, {1, theta, theta^2, ..., theta^{n-1} }
    end for;                                                                            // CoefficientsOfIntegralBasisElement[i] = [a_0, a_1, ..., a_{n-1}] where a_i is the coefficient of theta^i
                                                                                        // ie. if BasisElement[i]= 1/5*(theta^2 + 4*theta + 3), then CoefficientsOfIntegralBasisElement[i] = [ 3/5, 4/5, 1/5 ]
    
    DivisorsOfClassNumberOfK:= Divisors(ClassNumber(K));    // computes the divisors of the class number of K
...
s,t:= Signature(K);     // computes the number of embeddings of K, where s:= the number of real embeddings, K -> R; 2t = number of complex embeddings, K -> C
r:= s+t-1;      // determines the number of fundamental units 
...
//fprintf LogFile, "DivisorsOfClassNumberOfK = %o\n", DivisorsOfClassNumberOfK;
    ...
    FFF,rootsofginFFF:= SplittingField(g);  // computes the splitting field FFF of g over Q, the roots of g in FFF   // computes the splitting field of F
    OF:= MaximalOrder(FFF);         // computes the ring of integers of FFF, OF
...
We want to choose the precision for p_i-adic computations (padicprecision[i]) 
is about three times as large as the largest value of m that we expect will 
occur in the p_i-adic reduction step (Section 18).  m will be of a size that 
makes p_i^m \approx H_0^(v+r), so we want to take
padicprecision[i] \approx 2*(v+r)*Log(H_0)/Log(p[i]).  (We would be fine if 
instead of "four times as large" it was just "larger" than.  We go four times 
as large as a safety factor to avoid having to back up and do all the 
calculations again with an increased precision.)

We cannot calculate H_0 a priori.  But we know H_0 will be essentially of 
the size of that largest of the constants coming from using Yu's and 
Matveev's theorems to bound the relevant linear forms in logarithms.

The constant c_2 \cdot \min{ c_3^{\prime}, c_3^{prime}  } from Yu's 
theorem will be bounded by something of the size
...
print("Something is wrong. The required precision for the computation seems to be very large.");
...
// Decomposition of Primes 
    ...
    pp:= [];        // for each i, stores prime ideals [ pp[i][1], ..., pp[i][m[i]] ] of OK lying above the prime p[i]; 
                        // ie. (p[i])O_K = pp[i][1]^{e[i][1]} * \cdots * pp[i][m[i]]^{e[i][m[i]]}, where there are m[i] primes lying above p[i] in OK; e[i][j] is the ramification index of pp[i][j]
     
    m:= [];         // for each i, stores the number of prime ideals of OK lying above p[i]
    e:= [];         // for each i, stores the ramification indices [ e[i][1],...,e[i][m[i]] ] of the prime ideals pp[i][j] lying above p[i]
    f:= [];         // for each i, stores the residue degrees [ f[i][1],...,f[i][m[i]] ] of the prime ideals pp[i][j] lying above p[i]

    IndicesOfUnramifiedDegreeOnePrimesAbovep:= [];  // for each i, stores indices of prime ideals pp[i][j] above p[i] having e[i][j] = f[i][j] = 1
                                                        // ie. stores the indices of the unramified degree one prime ideals above p[i]
    h:= [];
    pi:= [];        // computes the least positive integer h[i][j] such that pp[i][j]^h[i][j] is principal and the generator pi[i][j] of pp[i][j]^h[i][j] 
                        // ie. for each i, given a prinme ideal pp[i][j] above p[i] with e[i][j] = f[i][j] = 1, computes the least positive integer h[i][j] such that pp[i][j]^h[i][j] is principal 
    DecompositionOfp:= [];  // for each i, stores prime ideal decomposition of p[i] in OK; ie. stores pp[i][j], e[i][j] where (p[i])O_K = pp[i][1]^{e[i][1]} * \cdots * pp[i][m[i]]^{e[i][m[i]]}
    for i in [1..v] do      // runs through each prime, p[i]
        pp[i]:= [];
        e[i]:= [];
        f[i]:= [];
        ...
        m[i]:= #DecompositionOfp[i];        // computes the number of distinct prime ideal factors pp[i][j] above p[i] in OK
        
        for j in [1..m[i]] do
            pp[i][j]:= DecompositionOfp[i][j][1];   // stores the jth prime ideal pp[i][j] in OK above p[i]
            e[i][j]:= DecompositionOfp[i][j][2];    // stores the ramification index of the jth prime ideal pp[i][j] above p[i]
            f[i][j]:= InertiaDegree(pp[i][j]);      // stores the residue degree of the jth prime ideal pp[i][j] above p[i]
            if e[i][j]*f[i][j] eq 1 then
                Append(~IndicesOfUnramifiedDegreeOnePrimesAbovep[i],j);
                k:= 1;
                while (IsPrincipal(pp[i][j]^DivisorsOfClassNumberOfK[k]) eq false) do 
                    k:= k + 1;      // runs through divisors of the class number h of K; the smallest integer h[i][j] such that pp[i][j] is principal necessarily divides the class number, h, of K
                end while;
...
// Irreducible Polynomial g   
    ...
    Kpp:= [];       // for each prime ideal pp[i][j] above each prime p[i], stores the completion of K at the prime ideal pp[i][j]
    mKpp:= [];      // stores the embedding mKpp[i][j]: K -> Kpp[i][j] for each pp[i][j]
    gp:=[* *];      // for each prime ideal pp[i][j], stores the corresponding factor, gp[i][j],  of g(t) in Q_{p[i]}[t]
    for i in [1..v] do
        Kpp[i]:= [];
        mKpp[i]:= [* *];
        gp[i]:= [];
        for j in [1..m[i]] do       // runs through the number of distinct prime factors of p[i] in OK, m[i]
            Kpp[i][j], mKpp[i][j]:= Completion(K,pp[i][j] : Precision :=padicprecision[i] );        // computes the completion of K at the prime ideal pp[i][j], and the embedding mKpp[i][j]: K -> Kpp[i][j]
            gp[i][j]:= MinimalPolynomial( mKpp[i][j](theta), PrimeField(Kpp[i][j]));                // determines gp[i][j]:= g_j(t), where g(t) = (g_1)* \cdots *(g_{m[i]}), where g_j in Q_{p[i]}[t]
                                                                                                        // ie. g_j(t) corresponds to the prime ideal pp[i][j] in (p[i])O_K = pp[i][1]^{e[i][1]}*\cdots*pp[i][m[i]]^{e[i][m[i]]}
        end for;                                                                                        // corresponds to the prime decomposition of p[i] in OK
    end for;

//============================================================================//
// Completion Of Ideals In Splitting Field   


    ppF:= [];       // for each i, stores a arbitrarily selected prime ideal of FFF, the splitting field of g, above p[i]
                        // ie. (p[i])OF = p[i][1]^{e[i][1]} * \cdots * p[i][m[i]]^{e[i][m[i]]}, where OF is the ring of integers of the splitting field, FFF
                        // stores ppF[i] = pp[i][j] for some j
                        
    eF:= [];        // for each i, stores the ramification index, eF[i], of ppF[i] over Q
                        // NB. since the splitting field is Galois over Q (ie. FFF/Q is Galois), every prime ideal of OF above p[i] has the same ramification index and residue degree
    fF:= [];        // for each i, stores the residue degree, fF[i], of ppF[i] over Q
    FFFppF:= [];    // for each i, for the selected prime ideal ppF[i] in FFF above p[i], stores the completion of FFF (the splitting field of g) at the prime ideal ppF[i]
                        // NB. FFFppF[i] contains the splitting field of g over Q_{p[i]}
    ...
    mFFFppF:= [* *];        // for each i, for the selected prime ideal ppF[i] in FFF above p[i], stores the embedding mFFFppF[i]: FFF -> FFFppF[i] 
    SS:= [];        // stores [FFFppF[i] : Q_p[i]] for each i
    DecompositionInFFFOfp:=[];      // for each i, stores the decomposition of p[i] in OF (the ring of integers of FFF)
...
// Theta_ij   
    ...
    thetap:=[];     // for each prime p[l], stores theta[l][i][j]:= theta_i^{(j)}, tje associated roots of g in the completion of Q_{p[l]}
    ...
    for l in [1..v] do
        ...
        for i in [1..m[l]] do
            thetap[l][i]:= [* *];
            temp:= Roots(gp[l][i],FFFppF[l]);       // computes roots of gp[l][i] = g_i in the (not quite) completion of Q_{p[l]} 
            for j in [1..#temp] do                  // #temp = degree of gp[l][i] = e[l][i]*f[l][i]
...
//============================================================================//
// Image Of Basis Elements Into Q_{p_L}

    ImageOfIntegralBasisElementp:= [];  // stores ImageOfIntegralBasisElementp[L][i][j][k]:= the image of the kth element in the integral basis for K under the embedding of K -> completion of Q_{p_L}, where
                                            // K -> completion of Q_{p_L}} is defined by the map theta -> thetap[L][i][j]
    for l in [1..v] do  // for each prime p[l]
        ImageOfIntegralBasisElementp[l]:=[];
        for i in [1..m[l]] do   // for each prime ideal pp[l][i] in OK above p[l]
            ImageOfIntegralBasisElementp[l][i]:= [];
            for j in [1..e[l][i]*f[l][i]] do    // for each root of the polynomial gp[l][i] associated to the prime ideal pp[l][i]
                                                    // ie. gp[l][i]:= g_i(t), where g(t) = (g_1)* \cdots *(g_{m[i]}), where g_ii in Q_{p[l]}[t]
                                                    // g_i(t) corresponds to the prime ideal pp[l][i] in (p[l])O_K = pp[i][1]^{e[i][1]}*\cdots*pp[i][m[i]]^{e[i][m[i]]}
                                                    // NB. deg(g_i(t)) = deg( gp[l][i] ) = e[l][i]*f[l][i] 
       
                ImageOfIntegralBasisElementp[l][i][j]:= [* *];
                for k in [1..n] do      // for each element in the integral basis of K; since [K:Q] = n, there are n such elements
                    ImageOfIntegralBasisElementp[l][i][j][k]:= 0;
                    for ii in [1..n] do         // runs through the representation of the kth element of the integral basis; 
                                                    // ie. each basis element is defined by CoefficientsOfIntegralBasisElement[k] = [a_0, a_1, ..., a_{n-1}] where a_i is the coefficient of theta^i
...
//============================================================================//
// Image Of Principal Ideal Generators pi[L][k] Into Q_{p_l}

    ImageOfpip:= [];    // stores ImageOfpip[L][k][l][i][j]:= the image of pi[L][k] under the embedding of K -> Completion of Q_{p_l} 
                             // K -> completion of Q_{p_l}} is defined by the map theta -> thetap[l][i][j]
                             // NB. pi[L][k] is the generator of the principal prime ideal pp[L][k]^h[L][k] above the prime p[L] in OK
    for L in [1..v] do // for each prime p[L]
        ImageOfpip[L]:= [];
        for k in [1..m[L]] do   // for each prime ideal pp[L][k] in OK above p[L]
            if e[L][k]*f[L][k] eq 1 then        // if the prime ideal pp[L][k] has e[L][k] = f[L][k] = 1
                ImageOfpip[L][k]:= [];
                for l in [1..v] do      // for each prime p[l]
                    ImageOfpip[L][k][l]:= [];
                    for i in [1..m[l]] do       // for each prime ideal pp[l][i] in OK above p[l]
                        ...
                        for j in [1..e[l][i]*f[l][i]] do        // for each root of the polynomial gp[l][i] associated to the prime ideal pp[l][i]
                            ...
                            for ii in [1..n] do         // runs through the image of each element of the integral basis of OK as embedded in K -> Completion of Q_{p_l}
                                ImageOfpip[L][k][l][i][j]:= ImageOfpip[L][k][l][i][j] + pi[L][k][ii]*ImageOfIntegralBasisElementp[l][i][j][ii];
                            end for;    // ends ii
                        end for;        // ends j
                    end for;    // ends i
                end for;        // ends l
            else
                ImageOfpip[L][k]:= [[[*0*]]];   // if the prime ideal pp[L][k] does not have e[L][k] = f[L][k] = 1, then it's exponent is determined via the PIRL
            end if;
        end for;        // ends k
    end for;    // ends L
...
CField<i>:= ComplexField();
ImageOfIntegralBasisElementC2:=[];
for i:=1 to n do
    tempf:= hom< K -> CField | thetaC[i] >;       // ADELA: ATTEMPT TO FIX MAPPING ELEMENTS TO 0 IN C EMBEDDING
    ImageOfIntegralBasisElementC2[i]:=[**];
    for k:=1 to n do
        ImageOfIntegralBasisElementC2[i][k]:=0;
        for ii:= 1 to n do
            ImageOfIntegralBasisElementC2[i][k]:=ImageOfIntegralBasisElementC2[i][k] + CoefficientsOfIntegralBasisElement[k][ii]*tempf(theta)^(ii-1);
        end for; //ii
    end for; //k
end for; //i 
...
////////////////////////////////////////////////////////////////////////
/*
ImageOfpiF[L][k][i] := the image of pi[L][k] under the embedding of K into F defined by the map theta --> thetaF[i].  
...
//============================================================================//
// The Prime Ideal Removing Lemma (PIRL)
//============================================================================//
...
//============================================================================//
// Procedures Needed for PIRL Algorithm 
//============================================================================//

//============================================================================//
// Build List Of Valid Tuples When pp[L][k] Is The Unconstrained Prime Above p[L]

/*
INPUT:
    i:= indexing integer element, beginning at i:= 1, where
        i runs through all elements D[i], where
            D[i]:= the upper bound on the exponent of the prime ideal pp[L][i] above p[L] in the factorization of bb = \mathfrak{b}
    L:= integer denoting the prime p[L] for which there are prime ideals above, pp[L][k] having unconstrained exponents
    k:= integer denoting the prime ideal pp[L][k] above p[L] in OK having e[L][k]=f[L][k] = 1 and unconstrained exponents
        ie. pp[L][k] satisfies Corollary 4.2
    aaa:= a tuple of nonnegative integers such that that PowerProduct(pp[L],aaa) is the factor of bb=\mathfrak{b} consisting of primes above p[L]
        NB. aaa[k] = 0 where k has the meaning that pp[L][k] is the unconstrained prime above p[L] (pp[L][k] must have e[L][k]=f[L][k]=1)
    D:= [ D[1], ..., D[m[L]], where D[i] runs through all prime ideals above p[L] and where
        D[i]:= the upper bound on the exponent of the prime ideal pp[L][i] above p[L] in the factorization of bb = \mathfrak{b}
    m:= [m[1], ..., m[L]] where
        m[i]:= the ith prime ideal pp[L][i] above p[L], where there are m[L] prime ideals above p[L] in OK
    CC:= array such that CC[i][j]:= e[L][i]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{j}^{(l)} )}
    LIST:= sequence of elements of the form [[k],aaa], where
        [k], aaa:= as above
...
*/
    ...
    procedure BuildListOfValidTuplesWhenppLkIsTheUnconstrainedPrimeAbovepL(i,L,k,~aaa,~D,~m,~CC,~LIST)
        ...
        for l in [0..Floor(D[i])] do   // runs through all possible exponents on the prime ideal pp[L][i], where these exponents are <= D[i]
                                    // NB. the unconstrained prime ideal has DD[i] ==  lower bound
            aaa[i]:= l;
            if i eq m[L] then
                Include(~LIST,[[k],aaa]);       // stores [[k],aaa] if the procedure has gone through all prime ideals above p[L] with the given choices of exponents
            else
                for j in [i+1..m[L]] do         // runs through the prime ideals appearing in order after p[L][i]
                    if aaa[i] gt CC[i][j] then          // compares the exponent aaa[i] to 
                        D[j]:= Min(D[j],CC[j][i]); 
                    end if;
                end for;
...
OUTPUT:
    CC:= array such that CC[i][j]:= e[L][i]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{j}^{(l)} )}
    DD:= array such that 
        if p[L] does not ramify completely in OK:
            DD[i]:= Floor( (1/2)*e[L]*ord_{p[L]}(DiscriminantOfg) ) 
        if p[L] ramifies completely in OK:
            DD[i]:= Floor( (1/(n-1))*ord_{p[L]}(DiscriminantOfg) ) 
    
COMMENTS:
    This algorithm is based primarily on Tzanakis, de Weger's "How to explicitly solve a Thue-Mahler equation", Ch. 5
    and Kyle Hambrook's Phd Thesis, Ch. 4
    
    Lemma 4.1: (a) 
        Let p[L] be a prime, pp[L][i] a prime ideal above p[L] in OK. For every pair i != j in {1, ..., m[L]}, if 
            ord_{pp[L][i]} > e[L][i]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{j}^{(l)} )}
        then
            ord_{pp[L][j]} <= e[L][j]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{j}^{(l)} )}
    Lemma 4.1 (b)
        For every i in {1,...,m[L]}, 
            ord_{pp[L][i]} <= e[L][i]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{i}^{(l)} )}
        NB. This case occurs only if e[L][i]=f[L][i] = 1, since deg(g_i) = e[L][i]f[L][i]
            Hence, if deg(g_i) = 1, there is only 1 root, theta_{i}^{(1)}, of g_i
            
    Corollary 4.2
        There is at most one pp[L][i] above p[L] in OK with
             ord_{pp[L][i]} >= Floor( (1/2)*e[L]*ord_{p[L]}(DiscriminantOfg) ) 
        if p[L] does not ramify completely in OK, and 
            ord_{pp[L][i]} >= Floor( (1/(n-1))*ord_{p[L]}(DiscriminantOfg) ) 
        if p[L] ramifies completely in OK.
        This prime ideal pp[L][i] necessarily satisfies e[L][i] = f[L][i] = 1
...
*/
    ...
    for L in [1..v] do  // for each prime p[L]
        ...
        DegF:= Degree( FFFppF[L], pAdicField(p[L]) );

        // Lemma 4.1 (a)
        CC:= [];        // stores CC[i][j]:= e[L][i]*min_{k,l} {ord_p( theta_{i}^{(k)} - theta_{j}^{(l)} )}
        for i in [1..m[L]] do   // for each prime ideal pp[L][i] in OK above p[L]
            CC[i]:= [];
            for j in [1..m[L]] do       // for each prime ideal pp[L][j] in OK above p[L]
                temp:= [];      // stores ord_{p[L]}( theta_i^{(k)} - theta_j^{(l)} ) for every pair i != j in {1, ..., m[L]}
                for k in [1..#thetap[L][i]] do  // for each theta_[L][i][k]:= theta_i^{(k)}, the associated roots of g in the completion of Q_{p[L]}
                    for l in [1..#thetap[L][j]] do      // for each theta_[L][j][l]:= theta_j^{(l)}, the associated roots of g in the completion of Q_{p[L]}
                        Append(~temp, ( (1/DegF )*( Valuation(Norm(thetap[L][i][k] - thetap[L][j][l],  pAdicField(p[L]) )) )) );
                    end for;
                end for;
        ...
        for i in [1..m[L]] do   // for each prime ideal pp[L][i] in OK above p[L]
            ...
            else        // if e[L][i] != 1 or f[L][i] != 1
                temp:= [];       // stores ord_{p[L]}( theta_i^{(k)} - theta_i^{(l)} ) for every i in {1, ..., m[L]}
                for k in [1..#thetap[L][i]] do  // for each theta_[L][i][k]:= theta_i^{(k)}, the associated roots of g in the completion of Q_{p[L]}
                    for l in [k+1..#thetap[L][i]] do    // for each theta_[L][i][l]:= theta_i^{(l)}, the associated roots of g in the completion of Q_{p[L]}
                        Append(~temp, ( (1/DegF )*( Valuation(Norm(thetap[L][i][k] - thetap[L][i][l],  pAdicField(p[L]))) )) ); 
                    end for;
                end for;
        ...
        DD:= [];        // stores Floor( (1/2)*e[L]*ord_{p[L]}(DiscriminantOfg) ) if p[L] does not ramify completely in OK, Floor( (1/(n-1))*ord_{p[L]}(DiscriminantOfg) ) if it does
        for i in [1..m[L]] do         // for each prime ideal pp[L][i] in OK above p[L]
            if (#e[L] eq 1) and (e[L][1] eq n) then     // if there is only 1 prime ideal pp[L][i] above p[L] and it ramifies completely in OK
                DD[i]:= ( (1/(n-1))*Valuation(DiscriminantOfg, p[L]) );
            else
                DD[i]:= ((1/2)*e[L][i]*Valuation(DiscriminantOfg, p[L]));
            end if;
        end for;
...
COMMENTS:
    ...
    For each unramified degree 1 prime pp[L][k] above p[L], this algorithm lists all valid tuples assuming pp[L][k] is the unconstrained prime ideal above p[L] dividing (x-y\theta).  
        ie. for i in [1,...,m[L]], i != k, assume the exponent of the prime ideal pp[L][i] above p[L] in the factorization of (x-y\theta) is 
            < (1/2)*max(e[L][1],...,e[L][m[L]])*ord_p(D_g).  
        That is, assume Corollary 4.2 holds for pp[L][k] so that ord_{pp[L][k]}(x-y\theta) > (1/2)*e[L][k]*ord_p(D_g).
        NB. Assume nothing about the exponent ord_pp[L][k](x-y\theta) of pp[L][k] in the factorization of (x-y\theta).
...
// COMMENT
        ...
        if #IndicesOfUnramifiedDegreeOnePrimesAbovep[L] gt 0 then       // if there are prime ideals pp[L][k] above p[L] having e[L][k] = f[L][k] = 1
                                                                        // these prime ideals are unconstrained; meaning that we may only bound their exponents from below
                                                                        // ie. we assume nothing about the exponent ord_{pp[L][k]}( x-y\theta )

            for kk in [1..#IndicesOfUnramifiedDegreeOnePrimesAbovep[L]] do  
                k:= IndicesOfUnramifiedDegreeOnePrimesAbovep[L][kk];    // for each prime ideal pp[L][k] above p[L] having e[L][k] = f[L][k] = 1
                
                D:= [];         // stores D[i]:= the upper bound on the exponent of the prime ideal pp[L][i] above p[L] in the factorization of bb = \mathfrak{b}

                for i in [1..k-1] do    // runs through the other prime ideals pp[L][i], i!=k, above p[L]
                    if e[L][i]*f[L][i] eq 1 then       
                        D[i]:= DD[i];   // assuming pp[L][kk] is the unconstrained prime ideal above p[L] having e[L][kk] = f[L][kk] = 1, which satisfies Cor. 4.2, 
                                            // any other prime ideal pp[L][i] with e[L][i] = f[L][i] = 1 is bounded above by DD[i]
                    else
                        D[i]:= Min(CC[i][i],DD[i]);     // assuming pp[L][kk] is the unconstrained prime ideal above p[L] having e[L][kk] = f[L][kk] = 1, which satisfies Cor. 4.2,
                                                            // any other prime ideal pp[L][i] with e[L][i]*f[L][i] > 1 is bounded above by Min(DD[i], CC[i][i]) = Min(Cor 4.2, Lemma 4.1(b))
                                                            // ie. assume that pp[L][kk] satisfies Cor. 4.2, and is not bounded above by Lemma 4.1(b)
                    end if;
                end for;

                D[k]:= DD[k] + 1;

                for i in [k+1..m[L]] do         // runs through the other prime ideals pp[L][i], i!=k, above p[L]
                    if e[L][i]*f[L][i] eq 1 then
                        D[i]:= DD[i];   // assuming pp[L][kk] is the unconstrained prime ideal above p[L] having e[L][kk] = f[L][kk] = 1, which satisfies Cor. 4.2, 
                                            // any other prime ideal pp[L][i] with e[L][i] = f[L][i] = 1 is bounded above by DD[i]
                    else
                ...
                aaa:= [];       // stores ??? 
                // populates ListOfTuplesOfExponentsForPrimesDividingbbAbovep[L] assuming pp[L][k]is the unconstrained prime ideal above p[L] dividing (x-y\theta)
...
PrintFile(LogFile, ListOfTuplesOfExponentsForPrimesDividingbbAbovep);
...
for j:=0 to epI[k] do
    ...
    if k eq #epI then
        ...
        if prod eq a or prod eq -a then 
            Append(~ListOfIdealsOfNorma,bbb); 
        end if;
    else
...
/////////////////////////////////////////////////
/*
Form the list of all units of finite order \zeta in OK 
(i.e, the list of all roots of unity \zeta in OK) 
...
// ADELA FUCKING AROUND - attempting to remove superfluous cases
//////////////////////////////////
NumberOfCases:=#ListOfCases;

fprintf LogFile, "Number of cases: %o\n", NumberOfCases;

CopyOfListOfCases:= ListOfCases;
ToRemove:= [];
...
//print "---------------------------------------------------------------------------------------";
//print "INTEGRAL BASIS:", IntegralBasis(K);
//print "COEFFICIENTS OF INTEGRAL BASIS:", CoefficientsOfIntegralBasisElement;
//print "IMAGE OF INT BASIS IN C:", ImageOfIntegralBasisElementC;
//print "ALPHA:", alpha;


ImageOfzetaC:=[ComplexField(100) | ]; 
for i:=1 to n do
ImageOfzetaC[i]:=0;
for ii:= 1 to n do
...
ImageOfalphaC2:=[ComplexField() | ]; 
for i:=1 to n do
    // print "i:", i;
    ImageOfalphaC[i]:=0;
    // ADELA'S ATTEMPT TO FIX THIS:
    ImageOfalphaC[i]:= &+[alpha[ii]*ImageOfIntegralBasisElementC2[i][ii] : ii in [1..n]];
    //ImageOfalphaC[i]:= &+[alpha[ii]*ImageOfIntegralBasisElementC[i][ii] : ii in [1..n]];
    
    //print "IMAGE OF ALPHA, 'fixed':", ImageOfalphaC2[i];
    // print "IMAGE OF ALPHA, 'otherwise':", ImageOfalphaC[i];
    
    /*
    for ii:= 1 to n do
        print "ii:", ii;
        print "CHECKING EQUALITY:", ImageOfalphaC[i] eq -alpha[ii]*ImageOfIntegralBasisElementC[i][ii];
                print "CHECKING SUM:", ImageOfalphaC[i] + alpha[ii]*ImageOfIntegralBasisElementC[i][ii];
        ImageOfalphaC[i]:=ImageOfalphaC[i] + alpha[ii]*ImageOfIntegralBasisElementC[i][ii];
        print "FIRST IMAGE OF ALPHAC", ImageOfalphaC[i]; 
...
// ADELA FUCKING AROUND AGAIN

/*
V:=ValuesOfn(ListOfCases[iiii], alpha, [ImageOfzetap, ImageOfalphap] , [v,r], [*m, e, f, kk, hh *],  thetap);
for l:= 1 to v do
    if V[l] eq 0 then
       //ListOfCases[iiii][l][1][1]:= -1;
    elif V[l] eq -100 then
        Append(~ToRemove, iiii);
    end if; 
end for;
*/
end for; //iiii
/*
// iiii is the problematic case to be removed in place of jjjj
Temp:= [];
for iiii:=1 to NumberOfCases do
    Large:= [];
    for l in [1..v] do
        if (ListOfCases[iiii][l][1][1] eq -1) then
            Append(~Large, l);
        
            for jjjj:=1 to NumberOfCases do
                if jjjj ne iiii then 
                    if &and[ListOfCases[iiii][i] eq ListOfCases[jjjj][i] : i in [1..v] | i notin Large ] eq true then
                        if &and[ListOfCases[iiii][i][2] eq ListOfCases[jjjj][i][2] and ListOfCases[iiii][i][3] eq ListOfCases[jjjj][i][3] : i in Large] then
                            if &and[ListOfCases[jjjj][l][1][1] ne -1 : l in Large] and &and[ListOfCases[jjjj][v+j] eq ListOfCases[iiii][v+j] : j in [1..2] ] then
                            Append(~Temp, [iiii, jjjj]);
                            end if;
                        end if;
                    end if;
                end if;
...
CasesToBeRemoved:= [];
for iiii in Temp do
    Append(~CasesToBeRemoved, ListOfCases[iiii[1]]);
end for;

for iiii in ToRemove do
    if ListOfCases[iiii] notin CasesToBeRemoved then
        Append(~CasesToBeRemoved, ListOfCases[iiii]);
    end if;
end for;

for Case in CasesToBeRemoved do
    Exclude(~ListOfCases, Case);
end for;
*/
NumberOfCases:=#ListOfCases;

fprintf LogFile, "Number of cases after ValueOfn reduction: %o\n", NumberOfCases;



// end of Adela's fuckery
///////////////////////////////////////////////////////
...
//print "2nd AlphaC:", ImageOfalphaC;
...
If we are NOT in the case where there are no unramified degree one primes above p[l], we fix the index i0 as in Section 7.  
We will represent i0 by the unique pair of integers (i,j) such that 
theta^{(i_0)} = theta_i^{(j)}.  
i0[l] will be a sequence of two integers such that
theta^{(i_0)} is represented by thetap[l][i0[l][1]][i0[l][2]]
...
if temp lt 0 then 
print "Case, Temp2:=", iiii, temp;
fprintf LogFile, "No solutions possible in case iiii = %o because Nstar[l] < 0, l = %o\n", iiii, l;
//continue iiii; 
end if; //no solutions in the current case
if (temp lt Nstar[l]) then 
    Nstar[l]:=temp; 
elif (temp lt 0) then
    Nstar[l]:= 0;
end if;
...
Find preimages in FFF of theta^{(i_0)}, theta^{(j)}, theta^{(k)} under the embedding sigma = mFFFppF[L] of FFF into FFFppF[l].  
We choose not to use MAGMA's bulit in preimage-finder to avoid round-off errors.
...
//print "MINMINMINMINMINmin alpha:", ImageOfalphaC[1];
//print "MINMINMINMINMINmin zeta:", ImageOfzetaC[1];
...
////////////////////////
/*
Find preimages in FFF of theta^{(i_0)}, theta^{(j)}, theta^{(k)} under the embedding sigma of FFF into \CC.  
We choose not to use MAGMA's bulit in preimage-finder to avoid round-off errors.

To find the preimage of theta^{(i_0)}, we actually find the index ii0 in 
{1,...,n} such that sigma(thetaF[ii0]) = thetaC[i0i0i0].
Similarly for theta^{(j)}, theta^{(k)}.
...
To find the preimage of theta^{(i_0)}, we actually find the index ii0 in {1,...,n} such that 
sigma(thetaF[ii0]) = thetaC[i0i0i0].
Similarly for theta^{(j)}, theta^{(k)}.
...
///////////////////////////////////////////////////////////////////////////
/* 
Start while loop that will increase m until the p_l-adic reduction yeilds a new 
upper bound for n_l or until a number of increases has been made with no success
*/
///////////////////////////////////////////////////////////////////////////
flag:=true;
RunThroughNumber:=0;
...
Input:=[*zeta,alpha,pi,eps,theta,J,ValueOfn,kk,hh,ss,tt,s,t,c,p,a,r,n,v,nJ,nQ,nJr*];
...
*/
//////////////////////////////////////////////////////////////////////////
// ADELA TESTING TO SEE IF SOLUTIONS CAN BE PRINTED AT EACH CASE

fprintf LogFile, "Case = %o\n", iiii;
fprintf LogFile, "Solutions = %o\n", Solutions;
fprintf LogFile, "=====================================================";

fprintf OutFile, "Case = %o\n", iiii;
fprintf OutFile, "ss = %o\n", ss;
fprintf OutFile, "Solutions = %o\n", Solutions;
fprintf OutFile, "=====================================================";
...
BETA1:= zeta*alpha;
for i:=1 to nJ do
BETA1 *:= pi[J[i]][kk[J[i]]]^bbb[i];
end for;
for i:=1 to r do
BETA1 *:= eps[i]^bbb[nJ+i];
end for;
...
intrinsic ValuesOfn(Case:: List, alpha:: RngOrdElt, ImageOfZetaAlphap:: SeqEnum , vr:: SeqEnum, mefkkhh:: List, thetap:: SeqEnum) -> SeqEnum
    {}

    ImageOfzetap:= ImageOfZetaAlphap[1];
    ImageOfalphap:= ImageOfZetaAlphap[2];
    m:= mefkkhh[1];
    e:= mefkkhh[2];
    f:= mefkkhh[3];
    kk:= mefkkhh[4];
    ...
    /*try to find a choice of j,k that gives ord_{p_l}(delta_1) \neq 0*/
        for i:=1 to m[l] do
            ...
            for j:=1 to e[l][i]*f[l][i] do
                ...
                if IsIntegral(tempValueOfnl) and tempValueOfnl ge 0 then 
                    ValueOfn[l]:=Integers() ! tempValueOfnl;
                else
                    ValueOfn[l]:= -100;
                end if; 
